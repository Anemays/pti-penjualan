...
class login extends CI_Controller
{
    ...
    function login_act()
    {
        ...
        $cekLoginUser = $this->m_user->getUser($email, $password);
        ...
        if ($this->input->method() != 'post') {
        ...
        } elseif ($cekLoginUser->num_rows() == NULL) {
            $this->session->set_flashdata('peringatan', 'Email tidak ditemukan');
            $cekEmailUser = $this->m_user->getEmailUser($email);

            if ($cekEmailUser->num_rows() == NULL) {
                $this->session->set_flashdata('peringatan', 'Email tidak ditemukan');
            } else {
                $this->session->set_flashdata('peringatan', 'Password salah');
            }
            ...
            foreach ($cekLoginUser->result() as $data) {
    ...
    function lupaPassword_act()
    {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $config = array(
                'protocol' => 'smtp',
                'smtp_host' => 'ssl://smtp.gmail.com', // smtp web
                'smtp_port' => '465', // port ssl/tsl
